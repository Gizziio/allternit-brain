...
- 2026-09-09 — rq-20260909-004 / HarnessRouter CE: decision **fork_reskin** (human redirect, supersedes same-day reverse_engineer) — fork HarnessRouter CE and strip the Docker packaging; folded INLINE into the ao v3 plan as P6 (UHP execution layer, Docker-free) + P7 (harness auto-install/onboarding). Docker was only the appliance layer (bundling + per-session-user isolation), not an architecture requirement. ao binary supervises the forked Python gateway+runner as loopback child processes — not embedded in the Rust binary. Provider routing stays in the Allternit gateway. Complements, does not replace, the ao v3 fork / native-sessions pickup.
