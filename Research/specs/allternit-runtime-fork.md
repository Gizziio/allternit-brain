...
- **P6 — UHP execution layer, Rust port (own spec):** implement HarnessRouter CE capability in Rust inside the ao binary (`rq-20260909-004`; human language call 2026-09-09 — HR CE is reference implementation + test oracle only, no shipped Python). UHP Responses surface, harness registry ("choose a brain"), sessions, per-backend drivers; P6a core + kimi/claude/codex, P6b remaining drivers gated on HR pytest behavior. Provider routing stays in the Allternit gateway. Evidence: plan doc §2.6. **Does not change P0 scope.**
