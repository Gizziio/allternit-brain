...
All 41 Stripe payment links now collect `custom_fields` (business_name + project_brief) at checkout and redirect to `/thank-you.html`. A Cloudflare Worker `allternit-services-hooks` (source in `allternit-websites/projects/services.allternit.com/worker/`, D1 database `allternit-services-bookings`) receives the `checkout.session.completed` webhook, tags/creates a Stripe Customer, logs the booking to D1, and emails `allternitpbc@gmail.com` via Cloudflare Email Routing.
